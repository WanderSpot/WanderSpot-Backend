...
public class JWTUtil {
    ...
    public String createAccessToken(MemberDto member) {
        return create(member, "access-token", accessTokenExpireTime);
    }

    //	AccessToken에 비해 유효기간을 길게 설정.
    public String createRefreshToken(MemberDto member) {
        return create(member, "refresh-token", refreshTokenExpireTime);
    }
...
//		jwt 토큰의 구성 : header + payload + signature
    private String create(MemberDto memberDto, String subject, long expireTime) {
...
//		저장할 data의 key, value
        ...
        claims.put("nickname", memberDto.getUserName());
...
//			parseClaimsJws : 파싱하여 원본 jws 만들기
            log.info("autorization is " + token);
...
//			Claims 는 Map 구현체 형태
    ...
    public String getUserId(String authorization) {
        ...
        try {
            log.info("autorization is " + authorization);
            String token = authorization.startsWith("Bearer ") ? authorization.substring(7) : authorization;
            claims = Jwts.parserBuilder().setSigningKey(this.generateKey()).build().parseClaimsJws(token);
        } catch (Exception e) {
